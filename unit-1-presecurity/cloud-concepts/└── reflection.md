...
After the session, I learned that the cloud is a universal service that can not only store data, but also provide services for processing and transmitting this data to the sender on-demand. However, when using such services, you need to be very careful, as they are not a "perfect lifeline." On the other hand, a very skilled scammer could be posing as a vendor and steal your data. To prevent such situations, always keep in mind the different service models (IaaS, PaaS, SaaS) and remember which parts of each model are controlled by the customer and which by the vendor.
