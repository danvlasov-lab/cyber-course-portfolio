# Assignment: Ethical and Legal Conduct in Tech and Cybersecurity

**Date:** 2026-09-24

**Source:** U2-05a Assignment: Ethical and Legal Conduct in Tech and Cybersecurity

**Environment:** My computer

## Goal
To work through five realistic ethical and legal dilemmas that you may actually encounter in entry-level tech work. For each, decide what you would do and why. At the end, write a short personal code of conduct committing to the principles you've identified.

## Scenario 1 - The colleague's password:

**What is happening (in my own words):**
Your college asked you kindly to log into her account with password is KevätTuuli2025 real quick and check ticket #4521 for her. She left her computer locked and she don't want to put the customer on hold.

**What's legally at stake:**
Legally, I'm not allowed to authorized to use another person's account by unauthorized access law. Sharing and using password also is not allowed.

**What's ethically at stake:**
Ethically, I should respect, because your college is really busy with other customers. You can help her and maybe later she will help you later.

**What I would do:**
I would tell her about rules, that I'm not allowed cannot log into her account. I would explain to her, that we will be fired If I'll use her login data. After that she probably be angry on me, but she will undestarnd. 

**What I would NOT do, and why:**
I would not actually log into her account and check her stuff. After that we can both be totally cooked and maybe even fired from our job.

**Who I would consult:**
I can ask HD-managers to log into my college account and check this ticket #4521. I would describe the whole situation and ask them how to solve it.

## Scenario 2 - The found credentials

**What is happening (in my own words):**
You're doing an office desktop service, changing a keyboard there, while manager is in a meting. You notice a note under the keyboard and there is an admin domain password (Welcome!2024Admin), that is quite old. Nobody is watching you.

**What's legally at stake:**
Legally I'm not allowed to use this admin password to log into managers accounts by unauthorized access law. On his desktop there are lot of confidential information, which is prohibited to see for employees.  

**What's ethically at stake:**
Ethically I also cannot use this password to log in, because I need to be honest. I need to give this password note back to manager, even if this password is not relevant already.

**What I would do:**
I would give this password note back to manager. Then I would kindly ask to change admin passwordб because someone could already copy this password and use it in a bad way. Also I would ask him to match this kind of data in other method(for example on phone).


**What I would NOT do, and why:**
I would not actually log in on admin account. After that I can reach huge consequences for my job and even can be fired.

**Who I would consult:**
I would contact to my manager. I would describe the whole situation and kindly ask him to change and save his password somewhere else. 

## Scenario 3 - The personal data peek

**What is happening (in my own words):**
You're working on thу helpdesk, and a friend from another department messages you to peek at his HR file. He wants to see what they have written it in his last review in the HR mailbox. 

**What's legally at stake:**
Legally I cannot view that data by GDPR law. Even If I have a access and permission, I'm not allowed to view and read this data. 

**What's ethically at stake:**
Ethically I might should check this info, but I need to respect his privacy info. This might break trust between us.

**What I would do:**
I would firstly say to my friend, that I'm not allowed to view his HR file, even if I have permissions on it. I would suggest that he contacts HR or her manager and ask them directly about the review.

**What I would NOT do, and why:**
I would not try to open HR mailbox, view and read this data. If I'm gonna do that, I might se stuff, that is not for me, and there will be huge consequences.

**Who I would consult:**
I would call supervisor, if I would't be sure about rules. I’m If everything i certain, I'd call to my friend directly.

## Scenario 4 - The vulnerability you accidentally noticed

**What is happening (in my own words):**
You're using your employer's cutomer portal as a customer. You notice that when you login, the URL contains ?userId=4827, and you change it to ?userId=4828. Suddenly, a different customer's order history loads and you quickly close the browser. No one saw what you did

**What's legally at stake:**
Legally I cannot just hide this secret by workplace norms and employer expectations law. I need to report about this to my managers. 

**What's ethically at stake:**
Ethically I also cannot hide this secret from other. This vulnerability may help to hackers in a future to actually hack our system and steal our data. 

**What I would do:**
I would ask my manager or IT-support to fix this vulnerability. I would describe the whole situation and ask thew to help. After that I would be less curious next time and just do my job.

**What I would NOT do, and why:**
I would not even try to check other customers histories. This breack confindentiallity of the customers and it may cause real problems

**Who I would consult:**
I would ask my manager or IT-support. They can help me with that situation.

## Scenario 5 - The off-hours request

**What is happening (in my own words):**
[2–3 sentences re-stating the situation, to confirm I understood it]

**What's legally at stake:**
[2–3 sentences naming the law or principle involved — unauthorized access, GDPR,
acceptable use policy, etc. Use the concepts from the background section.]

**What's ethically at stake:**
[2–3 sentences naming the ethical considerations — trust, harm, responsibility,
loyalty, etc. These are sometimes different from the legal considerations.]

**What I would do:**
[3–5 sentences describing your specific action. Not "I'd be careful" — what exact
steps would you take?]

**What I would NOT do, and why:**
[2–3 sentences naming what's tempting but you'd refuse to do.]

**Who I would consult:**
[1–2 sentences. Who in the organization (or outside it) would you bring this to,
and at what point? "No one" is rarely the right answer.]
